...
# Method Control Flags - Enable/Disable specific pruning methods
ENABLE_FULL_CONTEXT = True           # Always keep this True for baseline comparison
ENABLE_DOWNSAMPLED = True            # Downsampled context method
ENABLE_INTERPOLATED = True           # Interpolated context method  
ENABLE_TRUNCATED = True              # Truncated context method
ENABLE_UNCERTAINTY_METHODS = 1    # Enable uncertainty-based methods (computationally expensive)
ENABLE_UNCERTAINTY_SHORT = 1      # Uncertainty-based short context (requires ENABLE_UNCERTAINTY_METHODS)
ENABLE_UNCERTAINTY_INTERPOLATED = 1  # Uncertainty-based interpolated context (requires ENABLE_UNCERTAINTY_METHODS)

# Visualization Control Flags
ENABLE_DETAILED_PLOTS = True         # Enable detailed per-sample comparison plots
ENABLE_UNCERTAINTY_PLOTS = 1      # Enable uncertainty and delta analysis plots (requires ENABLE_UNCERTAINTY_METHODS)

# Data configuration
# CSV_PATH = "/home/sa53869/time-series/moirai/time-moe-eval/synthetic_sinusoidal.csv"
CSV_PATH = "/home/sa53869/time-series/moirai/time-moe-eval/ETT-small/ETTm1.csv"
...
print(f"\nEnabled Methods:")
print(f"  Full Context: {ENABLE_FULL_CONTEXT}")
print(f"  Downsampled: {ENABLE_DOWNSAMPLED}")
print(f"  Interpolated: {ENABLE_INTERPOLATED}")
print(f"  Truncated: {ENABLE_TRUNCATED}")
print(f"  Uncertainty Methods: {ENABLE_UNCERTAINTY_METHODS}")
if ENABLE_UNCERTAINTY_METHODS:
    print(f"    - Uncertainty Short: {ENABLE_UNCERTAINTY_SHORT}")
    print(f"    - Uncertainty Interpolated: {ENABLE_UNCERTAINTY_INTERPOLATED}")
print(f"  Detailed Plots: {ENABLE_DETAILED_PLOTS}")
print(f"  Uncertainty Plots: {ENABLE_UNCERTAINTY_PLOTS}")
...
# Create datasets for enabled pruning methods
if ENABLE_DOWNSAMPLED:
...
if ENABLE_UNCERTAINTY_METHODS:
    ...
    for i, (input_item, label_item, forecast) in enumerate(tqdm(zip(input_data_uncertainty_interpolated, label_data, forecasts_uncertainty_interpolated), desc="Processing uncertainty interpolated results", total=len(input_data_uncertainty_interpolated))):
        ...
        # Initialize empty results for disabled uncertainty methods
        sample_results_uncertainty_short = []
        sample_results_uncertainty_interpolated = []
        selected_samples_data = {}
...
# Always include full context as baseline
methods_summary['Full Context'] = np.mean([r['mae'] for r in sample_results])
methods_std_errors['Full Context'] = np.std([r['mae'] for r in sample_results]) / np.sqrt(len(sample_results))

# Add enabled methods conditionally
if ENABLE_DOWNSAMPLED:
    methods_summary['Downsampled (50%)'] = np.mean([r['mae'] for r in sample_results_downsampled])
    methods_std_errors['Downsampled (50%)'] = np.std([r['mae'] for r in sample_results_downsampled]) / np.sqrt(len(sample_results_downsampled))

if ENABLE_INTERPOLATED:
    methods_summary['Interpolated (50% replaced)'] = np.mean([r['mae'] for r in sample_results_interpolated])
    methods_std_errors['Interpolated (50% replaced)'] = np.std([r['mae'] for r in sample_results_interpolated]) / np.sqrt(len(sample_results_interpolated))

if ENABLE_TRUNCATED:
    methods_summary['Truncated (recent values)'] = np.mean([r['mae'] for r in sample_results_truncated])
    methods_std_errors['Truncated (recent values)'] = np.std([r['mae'] for r in sample_results_truncated]) / np.sqrt(len(sample_results_truncated))

if ENABLE_UNCERTAINTY_METHODS and ENABLE_UNCERTAINTY_SHORT:
    methods_summary['Uncertainty-based Short (50% most important)'] = np.mean([r['mae'] for r in sample_results_uncertainty_short])
    methods_std_errors['Uncertainty-based Short (50% most important)'] = np.std([r['mae'] for r in sample_results_uncertainty_short]) / np.sqrt(len(sample_results_uncertainty_short))

if ENABLE_UNCERTAINTY_METHODS and ENABLE_UNCERTAINTY_INTERPOLATED:
    methods_summary['Uncertainty-based Interpolated (50% replaced)'] = np.mean([r['mae'] for r in sample_results_uncertainty_interpolated])
    methods_std_errors['Uncertainty-based Interpolated (50% replaced)'] = np.std([r['mae'] for r in sample_results_uncertainty_interpolated]) / np.sqrt(len(sample_results_uncertainty_interpolated))
...
if ENABLE_DETAILED_PLOTS:
    # For 3 random samples, plot context, ground truth and prediction for all enabled methods
    num_samples = 3
    ...
    for plot_idx, idx in enumerate(sample_indices, 1):
        ...
        # Set x-axis limits to match other methods (based on full context length)
...
if ENABLE_UNCERTAINTY_METHODS and ENABLE_UNCERTAINTY_PLOTS:
